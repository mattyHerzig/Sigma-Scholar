...
def process_link(link):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_blacklink, link, top) for link in blacklink_links]
        for future in concurrent.futures.as_completed(futures):
            result = future.result()
            results.extend(result)  # Extend the list with individual strings
...
def process_blacklink(blacklink, top):
    if "military" not in blacklink:
        ...
        if len(table_links)>0:
            low = get_subdirectories(table_links[0])[-1]
        else:
            low=""
    else:
        mid = get_subdirectories(blacklink)[-1]
    ...
    result = f"{top} : {mid} : {low}"
    print(result)  # Print progress while running
    return [result]  # Return as a list of strings


links = get_links_from_webpage('https://www.scholarships.com/financial-aid/college-scholarships/scholarship-directory')

with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:  # Increase the number of threads
