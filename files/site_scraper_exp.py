...
def get_all_sublinks(link):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        # Use ThreadPoolExecutor to fetch links concurrently
        blacklink_links = list(executor.map(get_links_with_blacklink_class, top_links))
        for blacklink_list in blacklink_links:
            all_links.extend(blacklink_list)
            print(f"Completed fetching links layer 2: {len(all_links)} links so far.")
            
            table_links = list(executor.map(get_links_from_table, blacklink_list))
            for table_link_list in table_links:
                all_links.extend(table_link_list)
                print(f"Completed fetching links layer 3: {len(all_links)} links so far.")
...
def get_all_docs(links):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        # Use ThreadPoolExecutor to extract information concurrently
        outputs = list(executor.map(extract_page, links))
        print(f"Completed extracting information for {len(outputs)} pages.")
...
all_docs = get_all_docs(all_links)
